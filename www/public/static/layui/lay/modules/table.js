...
layui.define(['laytpl', 'laypage', 'layer', 'form'], function(exports){
  ...
  Class.prototype.syncCheckAll = function(){
    ...
    if(table.checkStatus(that.config['data']).isAll){
  ...
  Class.prototype.events = function(){
    ...
    that.elem.on('click', 'input[name="layTableCheckbox"]+', function(){
      ...
      //全选
      if(isAll){
        layui.each(that.config['data'],function(index,item){
            item[that.config.checkName] = checked;
        });
  ...
  //表格选中状态
  table.checkStatus = function(data){
    var nums = 0
    ,arr = []
    ;
    //,data = table.cache[id]
