...
require(['jquery'],function ($) {
    ...
    window.WkkyView = function () {
        //可能包含其他view
        this.childView = [];
    };

    window.WkkyView.prototype = {
        constructor : window.View,
        init:function () {
    ...
    window.Wkky = function (wkkyView,wkkyData) {
        this.wkkyView = wkkyView;
        this.wkkyData = wkkyData;
    };

    window.Wkky.prototype = {
        constructor:window.Wkky,
        update:function () {
            var _this = this;
            this.wkkyData.getDataFormRemote();
        },
        getContent:function () {
            return this.wkkyView.getContent();
        },
        getData:function () {
            return this.wkkyData.getData();
        }
    };

    var data = new window.WkkyData('/index/data');
    data.setOnBefor(function () {
        
    });
    data.setOnSuccess(function (handleResponse) {
        console.log(handleResponse.getData());
        console.log(handleResponse.getMeta());
        console.log(handleResponse);
    });
    data.setOnFail(function (handleResponse) {
        console.error('操作失败');
    });
    data.setOnError(function (e) {
        console.error('请求出错');
    });
    data.setOnAfter(function (e) {
        console.log(e);
    });
    data.getDataFormRemote();
});